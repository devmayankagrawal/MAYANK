trigger AccountTriggerToOppSelf on Account (before update) {
    ...
    if(!accMap.isEmpty()){
        for(Opportunity objOpp : [select Id, Name, Amount, Description, AccountId from Opportunity where AccountId IN : accMap.keySet()]){
            oppList.add(objOpp);
        }
    }
