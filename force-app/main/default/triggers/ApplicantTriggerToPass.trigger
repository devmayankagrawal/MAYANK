trigger ApplicantTriggerToPass on Applicant__c (before update) {
    ...
    if(!appMap.isEmpty()){
        for(Passport__c objPass : [select Id,Passport_Number__c,Status__c,Issue_Date__c,Applicant__c from Passport__c where Applicant__c IN : appMap.keySet()]){
    ...
    if(!passList.isEmpty()){
        for(Passport__c objPass : passList){
            if(appMap.containsKey(objPass.Applicant__c)){
                if((trigger.oldMap.get(objPass.Applicant__c).Gender__c=='Female') && (objPass.Status__c =='Rejected') && (objPass.Issue_Date__c.month()== 4)){
